...
// {   {0x04, 0x05, 0x04, 0x05, 0x04, 0x05}, {0x04, 0x05, 0x04, 0x05, 0x04,
// 0x05}, {0x04, 0x05, 0x04, 0x05, 0x04, 0x05}, {0x04, 0x05, 0x04, 0x05, 0x04,
// 0x05}, {0x04, 0x05, 0x04, 0x05, 0x04, 0x05},
//     {0x04, 0x05, 0x04, 0x05, 0x04, 0x05}, {0x04, 0x05, 0x04, 0x05, 0x04,
//     0x05}, {0x04, 0x05, 0x04, 0x05, 0x04, 0x05}, {0x04, 0x05, 0x04, 0x05,
...
void configureEndpointZero() {
  // This endpoint actually gets configured by default (with values seen below),
  // but this code could be used to configure other endpoints
...
void usb_send_descriptor(
    uint8_t descriptor[],
    uint8_t desc_bytes) { // See page 275 for rough outline of algorithm
  ...
  *UEINTX &= ~(1 << RXSTPI);
  ...
  if ((bmRequestType & 0x60) == 0) {
    ...
    case 0x00:     // GET_STATUS/
      *UEDATX = 0; // Send back 16 Bit for status (Not self powered, nowakeup,
                   // not halted)
      *UEDATX = 0;
      *UEINTX &= ~(1 << TXINI); // Send data (ACK) and clear FIFO
      while (!(*UEINTX & (1 << RXOUTI)))
        ;                        // wait for ZLP packet from host (ack)
      *UEINTX &= ~(1 << RXOUTI); // clear interrupt
      break;
    case 0x05: // SET_ADDRESS (procedure laid out in datasheet)
      *UDADDR =
          (wValue_l & 0x7F); // Save assigned address at UADD  (ADDEN = 0 still)
...
void encode(char message[], uint16_t chars[], uint16_t modifiers[],
  ...
  char special_buffer[21]; // Max length permitted for "special" length, used
                           // elsewhere (really it's 20, but add one space for
                           // the null terminator I'll need elsewhere)
  uint8_t special_buffer_length = 0;
  uint8_t new_length = 0;          // New length temp variable
  uint8_t special_modifier = 0x00; // used to apply special keys to all entries
                                   // within tag (<Shift> ... </Shift>)
  // Cases:
  //  Uppercase Letter
  //  Lowercase Letter
  //  Number
  ...
  for (int i = 0; i < *length; i++) {
    ...
    if (message[i] >= 'a' && message[i] <= 'z') { // a-z
      chars[new_length] =
          message[i] -
          0x5D; // Substract 0x61 to get from ASCII code to (# in the alphabet),
                // then add 0x04 to get into letter range in USB table
      new_length += 1;
      continue;
    }
    if (message[i] >= '1' && message[i] <= '9') { // 1-9
      chars[new_length] =
          message[i] - 0x13; // Substract 0x31 to remove ASCII code, then add
                             // 0x1E to get into USB number range
      new_length += 1;
      continue;
    }
    if (message[i] == '0') { // 0
    ...
    if (message[i] == '-') {
      chars[new_length] = 0x2D;
      new_length += 1;
      continue;
    }
    if (message[i] == '_') {
      chars[new_length] = 0x2D;
      modifiers[new_length] |= (1 << 1);
      new_length += 1;
      continue;
    }
    if (message[i] == '=') {
      chars[new_length] = 0x2E;
      new_length += 1;
      continue;
    }
    if (message[i] == '+') {
    ...
    // if (message[i] == '<') {chars[new_length] = 0x36; modifiers[new_length]
    // |= (1 << 1); new_length += 1; continue;}
    if (message[i] == '.') {
      chars[new_length] = 0x37;
      new_length += 1;
      continue;
    }
    if (message[i] == '>') {
    ...
    if (message[i] == '!') {
      chars[new_length] = 0x1E;
      modifiers[new_length] |= (1 << 1);
      new_length += 1;
      continue;
    }
    if (message[i] == '@') {
      chars[new_length] = 0x1F;
      modifiers[new_length] |= (1 << 1);
      new_length += 1;
      continue;
    }
    if (message[i] == '#') {
      chars[new_length] = 0x20;
      modifiers[new_length] |= (1 << 1);
      new_length += 1;
      continue;
    }
    if (message[i] == '$') {
      chars[new_length] = 0x21;
      modifiers[new_length] |= (1 << 1);
      new_length += 1;
      continue;
    }
    if (message[i] == '%') {
      chars[new_length] = 0x22;
      modifiers[new_length] |= (1 << 1);
      new_length += 1;
      continue;
    }
    if (message[i] == '^') {
      chars[new_length] = 0x23;
      modifiers[new_length] |= (1 << 1);
      new_length += 1;
      continue;
    }
    if (message[i] == '&') {
      chars[new_length] = 0x24;
      modifiers[new_length] |= (1 << 1);
      new_length += 1;
      continue;
    }
    if (message[i] == '*') {
      chars[new_length] = 0x25;
      modifiers[new_length] |= (1 << 1);
      new_length += 1;
      continue;
    }
    if (message[i] == '(') {
      chars[new_length] = 0x26;
      modifiers[new_length] |= (1 << 1);
      new_length += 1;
      continue;
    }
    ...
    // When we get to a special, we will enter a for loop which picks up at next
    // char and continues until we reach '>'. Then we will adjust the *length
    // variable of the for loop to reflect the now shorter message
    if (message[i] == '<') {
      for (int j = 1;; j++) {
        if (message[i + j] == '>') {
          special_buffer_length = j - 1;
          break;
        } // We've reached end of tag
        special_buffer[j - 1] = message[i + j];
        if (j > 20) {
          special_buffer_length = -1;
          break;
        } // Tag is too long, must not be a special just a regular '<'
      }
      ...
      if (special_buffer_length == -1) {
        ...
        for (int x = 0; x < 21; x++) {
          special_buffer[x] = 0;
        } // clear array, probably not strictly necessary
        continue;
      }
...
void encode_keypresses(uint8_t *combinedMatrix, uint16_t *pressed_keys,
  ...
  uint8_t macros_to_send[8] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
                               0xFF, 0xFF, 0xFF}; // Start out as all blanks
  ...
  *modifier = 0x00;
  ...
  for (int inPin = 0; inPin < 2 * NUM_IN_PIN; inPin++) {
    for (int outPin = 0; outPin < NUM_OUT_PIN; outPin++) {
      if ((combinedMatrix[inPin] >> outPin) & 0x1) {       // If key is pressed
        ...
        if (numPressed >
            6) { // Too many keys pressed, error out (send 6x
                 // "keyboard_pressed_keys" values and 0x0 modifier)
          *modifier = 0;
          for (int i = 0; i < 6; i++) {
            pressed_keys[i] = 0x01;
          }
  ...
  // We have some macros to send
  if (macro_count) {
    // Save old pressed keys
    for (int i = 0; i < 6; i++) {
      old_pressed_keys[i] = pressed_keys[i];
    }
    // Save old modifier byte
    old_modifier = *modifier;

    // Send macros
    for (int i = 0; i < macro_count; i++) {
      // Get char* to macro string to send
      send_message(macros[macros_to_send[i] - 0xE8]);
...
ISR(USB_ENP_I) {
  if (*UEINT & (1 << 0)) { // We're talking about ep0?
    *UENUM = 0;
    if (*UEINTX & (1 << RXSTPI)) { // Setup packet recived, so lets reply to it
                                   // appropriately
      usb_ep0_setup();
    } // endif ep0
  } else {
...
ISR(USB_GEN_I) {
  if (*UDINT & (1 << EORSTI)) { // Was it an end-of-reset interrupt?
    *UDINT &= ~(1 << EORSTI);   // Clear EOR interrupt
    configureEndpointZero();  // Reconfigure endpoint (though ep0 should always
                              // be configured)
    *UENUM = 0;               // Select USB endpoint
    *UEIENX |= (1 << RXSTPE); // Enable setup packet receipt interrupts to
                              // process any setup packets received
  }
}
